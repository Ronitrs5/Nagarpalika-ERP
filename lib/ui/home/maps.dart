...
class _MapState extends State<MyMap> {
  ...
  void _onGetLocation() async {
    ...
    _markers.clear();
    setState(() {
      var latitude=_lastMapPosition.latitude;
      var longitude=_lastMapPosition.longitude;
      _markers.add(
          Marker(
            markerId: MarkerId(_lastMapPosition.toString()),
            position: _lastMapPosition,
  ...
  _onCameraMove(CameraPosition position) {
    ...
    setState(() {
      mapLat=_lastMapPosition.latitude;
      mapLon=_lastMapPosition.longitude;
      _markers.add(
          Marker(
            markerId: MarkerId(_lastMapPosition.toString()),
            position: _lastMapPosition,
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                ElevatedButton(
                  onPressed: () {
                    if (mapLon != null && mapLon!=null) {
                      Navigator.pop(
                        context,
                        LatLng(mapLat, mapLon),
                      );
                    } else {
                      Navigator.pop(context); // No location selected, simply go back
                    }
                  },
